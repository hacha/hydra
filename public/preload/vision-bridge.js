...
// 現在 hydra が描画している canvas をそのまま各 HMD に送り返す hub。
...
if (!hiddenVideos) {
  ...
  document.body.appendChild(hiddenVideos)
}
...
const vb = await VB.connect({ signaling: signalingUrl, id })
...
vb.onIncoming(async (peer, offer) => {
  ...
  const track = outputStream.getVideoTracks()[0].clone()
  await peer.acceptOffer(offer, [track])
  log('answered', peer.remoteId, '→ sending hydra canvas')
...
log('hub ready — waiting for HMD peers. s0=hmd1, s1=hmd2。例: src(s0).blend(s1).out(o0)')
